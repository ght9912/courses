                    ...
                    <tbody>
                        @foreach ($settings as $s)
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex">
                                        <button type="button" class="btn btn-primary" onclick="editMain({{$s->id}},this)">Editar</button>
                                        {{-- <button type="button" name="" id="" class="btn btn-danger" onclick="deleteMain({{$s->id}},this)">Eliminar</button> --}}
                                    </div>
                                </td>
                            </tr>
                        @endforeach
            ...
            <div class="modal-body">
                <div class="container-fluid">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="titulo" id="titulo" placeholder="Aqui escribe tu nombre">
                      </div>
                      <div class="mb-3">
                        <label for="subtitulo" class="form-label">Subtitulo</label>
                        <input type="text" class="form-control" name="subtitulo" id="subtitulo" placeholder="Aqui escribe tu email">
                      </div>

                      <div class="mb-3">
                        <label for="categoria" class="form-label">Contenido</label>
                        <textarea name="contenido" id="contenido" cols="30" rows="10" class="form-control" placeholder="modifica contenido"></textarea>
                      </div>
...
@section("scripts")
    <script >
        const mainData = {{ Js::from($settings)}}
        const mainRecursos = {{ Js::from($recursos)}}
        const URLBase="settings"
        ...
        const editMain = (id,el) =>{
            const setting = findMain(id);
            if(setting==undefined){return}
            console.log(setting);
            $("#titulo").val(setting.titulo);
            $("#subtitulo").val(setting.subtitulo);
            $("#contenido").val(setting.contenido);
            $("#formAdd").data("update",1)
            $("#formAdd").data("id",setting.id);
            $("#modalTitleId").html("Actualizar pagina "+setting.pagina)
            $("#modalAdd").modal("show");
            ...
            if($("#formAdd").data("update")==1){
                ...
                axios.post(URLBase+"/"+id,data).then((e)=>{
                   ...
                   $("#modalAdd").modal("hide")
                    ...
                    dt.row($('.update-'+id)).data([r.id,r.pagina,r.titulo,r.subtitulo,r.contenido,buttons ]).draw()
